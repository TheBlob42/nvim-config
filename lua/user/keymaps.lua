...
vim.api.nvim_create_autocmd('BufEnter', {
    callback = function(opts)
        if vim.api.nvim_buf_is_valid(opts.buf) then
            if not pcall(vim.api.nvim_buf_get_var, opts.buf, 'gradle-keybindings') then
                vim.api.nvim_buf_set_var(opts.buf, 'gradle-keybindings', true) -- set buf local variable to avoid resetting the keybindings
